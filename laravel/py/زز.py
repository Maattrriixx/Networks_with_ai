...
def get_room_for_point(rooms, x, y):
    ...
    # First try exact polygon containment
    for room in rooms:
        corners = room.get('corners', [])
        if len(corners) >= 3 and point_in_polygon(x, y, corners):
            return room['id'], room['type']  # type is used as room name
    # If no polygon, fallback to nearest by center distance
    best_room = None
    best_dist = float('inf')
    for room in rooms:
...
class WiredGA:
    def __init__(self, clusters, centroids, rooms, scale=0.05, max_cable_m=90.0,
        ...
        # Calculate maximum possible switches per cluster based on total ports needed
...
# ============================================================================
# Device Generation (with room assignment)
# ============================================================================
def generate_devices(rooms, clusters, centroids, best_switches, scale=0.05):
    ...
    # Helper to get room info for a point
    def get_room_info(x, y):
        rid, rname = get_room_for_point(rooms, x, y)
        return rid, rname
    ...
    # Access Switches and Patch Panels per cluster based on best_switches
    access_switches = []
    patch_panels = []
    for cid, sw_count in enumerate(best_switches):
        ...
        for i in range(actual_sw_count):
            ...
            pp_x = sw_x - 30
            pp_y = sw_y - 20
            # Room for patch panel and access switch
            pp_room_id, pp_room_name = get_room_info(pp_x, pp_y)
            sw_room_id, sw_room_name = get_room_info(sw_x, sw_y)
    ...
    # Core infrastructure - assign to a "Server Room" or nearest room
    ...
    # Try to find a room named "Server Room", otherwise nearest room
    server_room = None
    for r in rooms:
    ...
    if server_room:
        ...
        core_room_id, core_room_name = get_room_info(core_x, core_y)
    ...
    router_room_id, router_room_name = get_room_info(core_x, core_y)
    router = {
        'device_id': device_id,
        'type': 'Router',
        'x': round(core_x, 2),
    ...
    fw_room_id, fw_room_name = get_room_info(core_x + 30, core_y + 30)
    firewall = {
        'device_id': device_id,
        'type': 'Firewall',
        'x': round(core_x + 30, 2),
    ...
    srv_room_id, srv_room_name = get_room_info(core_x - 40, core_y - 20)
    server = {
        'device_id': device_id,
        'type': 'Server',
        'role': 'DC/DHCP/DNS',
    ...
    ups_room_id, ups_room_name = get_room_info(core_x + 60, core_y - 50)
    devices.append({
        'device_id': device_id,
        'type': 'UPS',
        'capacity_kva': 3.0,
...
# ============================================================================
# Connection Builder (unchanged except room assignment not needed)
# ============================================================================
def build_connections(devices, access_switches, patch_panels, room_outlets, room_cameras, core_sw_id, scale=0.05):
    ...
    # Data Outlets and Cameras -> nearest Patch Panel
