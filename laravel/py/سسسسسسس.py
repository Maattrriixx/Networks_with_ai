...
def load_rooms(rooms_list, scale=0.05):
    """Load rooms from Laravel request, keeping original IDs."""
    result = []
    for r in rooms_list:
        # Keep original id as is
        room_id = r.get('id')
        ...
        result.append({
            'id': room_id,  # same id as received from Laravel
            'type': rtype,
            'center': r['center'],
            'corners': corners,
...
# ============================================================================
# Clustering (using spatial proximity)
# ============================================================================
...
# ============================================================================
# Device Generation (Using original room IDs from Laravel)
# ============================================================================
def generate_devices(rooms, clusters, centroids, best_switches, scale=0.05):
    ...
    # Create a mapping from room index to room object
    rooms_by_index = {i: room for i, room in enumerate(rooms)}

    # Generate Data Outlets and Cameras per room (using original room ID)
    for room in rooms:
        room_id = room['id']   # This is the original ID from Laravel
        ...
        for i in range(n_ports):
            ...
            devices.append({
                'device_id': device_id,
                'type': 'Data Outlet',
                'room_id': room_id,      # same as Laravel ID
                'room_name': room['type'],
                'x': x,
                'y': y,
                'connectivity': 'wired',
    ...
    for cid, sw_count in enumerate(best_switches):
        ...
        for i in range(actual_sw_count):
            ...
            # For switches and patch panels, we assign the room ID of the first room in the cluster
            # (or you can leave null if not inside a specific room). This ensures no new IDs are generated.
            first_room_idx = clusters[cid][0]
            cluster_room_id = rooms[first_room_idx]['id']   # original Laravel ID
            ...
            devices.append({
                ...
                'cluster_id': cid,
                'room_id': cluster_room_id,    # using existing room ID from cluster
                'room_name': rooms[first_room_idx]['type'],
                'x': round(pp_x, 2),
                'y': round(pp_y, 2),
                'connectivity': 'wired',
    ...
    # Core infrastructure - assign to a "Server Room" if exists, otherwise first room's ID
    ...
    # Find a room of type 'Server Room' to place core devices, otherwise use first room
    server_room = None
    for r in rooms:
        if r['type'].lower() == 'server room':
            ...
            break
    if server_room:
        core_room_id = server_room['id']
        core_room_name = server_room['type']
    else:
        core_room_id = rooms[0]['id']
        core_room_name = rooms[0]['type']
    ...
    core_sw = {
        ...
        'x': round(core_x, 2),
        'y': round(core_y - 30, 2),
        'room_id': core_room_id,   # existing ID from Laravel
...
# ============================================================================
# Connection Builder (unchanged, but uses devices with room_id)
# ============================================================================
...
def run_complete_optimizer(rooms_list, scale=0.05, eps=200):
    # Load rooms with original IDs from Laravel
    ...
    # Return the rooms as they were received (with computed fields)
    return {
        'devices': devices,
        'connections': connections,
        'rooms': rooms,   # same IDs as Laravel sent
        'metadata': {
